...
<?php
    if(isset($_GET['u_id'])){
            ...
            $user_to_msg = isset($row_user['user_id']) ? ($row_user['user_id']) : '3';
            $user_to_name = isset($row_user['user_name']) ? ($row_user['user_name']) : 'T';
...
?>
    ...
    <div class="col-sm-3">
        ...
        if(isset($_GET['u_id'])){
            ...
            $user_id = isset($row['user_id']) ? ($row['user_id']) : '3';
            $user_name = isset($row['user_name']) ? ($row['user_name']) : 'T';
            $f_name = isset($row['f_name']) ? ($row['f_name']) : 'T';
            $l_name = isset($row['l_name']) ? ($row['l_name']) : 'Sherlock';
            $describe_user = isset($row['describe_user']) ? ($row['describe_user']) : 'Funny';
            $user_country = isset($row['user_country']) ? ($row['user_country']) : 'Vietnam';
            $user_image = isset($row['user_image']) ? ($row['user_image']) : 'null';
            $register_date = isset($row['register_date']) ? ($row['register_date']) : 'null';
